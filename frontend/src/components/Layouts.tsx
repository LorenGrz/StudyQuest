...
export function BottomNav() {
  ...
  return (
    <nav className="w-full h-[70px] bg-[#13131f]/95 backdrop-blur-md border-t border-white/8 flex justify-around items-center shrink-0 pb-[env(safe-area-inset-bottom)] z-50">
      {tabs.map((tab) => {
        ...
        return (
          <Link
            key={tab.path}
            to={tab.path}
            className={`flex flex-col items-center gap-1 text-[#8888aa] transition-all duration-200 flex-1 py-2 active:scale-95 ${isActive ? 'text-[#9d5df7]' : ''}`}
          >
...
export function AuthLayout({ children }: Props) {
  return (
    <div className="h-full flex items-center justify-center p-4 bg-gradient-to-t from-[#0d0d18] via-[#0d0d18] to-purple-950/15 overflow-y-auto">
      <div className="w-full max-w-[420px] bg-[#13131f] border border-white/8 rounded-3xl p-8 flex flex-col gap-6">
        <div className="text-center">
          ...
          <p className="text-[#8888aa] text-sm mt-1">Estudia. Compite. Gana.</p>
        </div>
        {children}
      </div>
    </div>
...
export function GameLayout({ children }: Props) {
  return (
    <div className="h-full bg-gradient-to-b from-[#0d0d18] to-[#1a0a2e] flex flex-col max-w-[480px] mx-auto self-center w-full overflow-y-auto">
      {children}
    </div>
  )
}
